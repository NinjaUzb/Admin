...
function Login() {
  ...
  function Login () {
    axios.post('https://api.escuelajs.co/api/v1/auth/login', login).then(data => console.log(data)
        ...
        <div className="right p-5">
          ...
          <form action="login" className='w-100 pt-4 d-flex flex-column gap-4'>
              <div className="username w-100 gap-2 d-flex flex-column">
                <label className='login-label fw-light ms-1' htmlFor="login">Login</label>
                <Input onChange={(e)=> handle(e)} id='email' value={login.email} className='login-input' placeholder='Email or phone number' />
              </div>
              <div className="password w-100 gap-2 d-flex flex-column">
                ...
                <Input.Password onChange={(e)=>handle(e)} id='password' value={login.password} className='login-input' placeholder="Enter password" />
              </div>
